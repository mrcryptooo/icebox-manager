const MSG = {
  ...
  accountingReport: (report, startJalali, endJalali) => {
    ...
    const sumLines  = [
      ...
      `  _(شامل: مخارج + خرید نقدی + پرداخت تأمین‌کننده + حقوق + برداشت + مصرف داخلی)_`,
      `${thin}`,
      ...
      `  _(پاداش‌های ثبت‌شده در مانده پرسنل لحاظ شده)_`,
      `${thin}`,
      `${afterSign} *مانده بعد از تعهدات: ${fmt(summary.afterObligations)} تومان*`,
      `${sep}`,
      `⚠️ _این گزارش برای مدیریت داخلی است و گزارش رسمی مالیاتی محسوب نمی‌شود._`,
    ];
