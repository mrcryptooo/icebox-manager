...
## پنل مدیریت کل سیستم — Super Admin Dashboard (Phase 9B)

### دسترسی

فقط `super_admin` (همان `OWNER_ID`) می‌تواند به این بخش دسترسی داشته باشد.
اگر `business_owner`، `manager`، `accountant` یا `staff` آدرس را دستی وارد کنند، **403 فارسی** می‌بینند:
...
| مسیر | توضیح |
|------|-------|
| `GET /admin` | داشبورد کل سیستم — کارت‌های خلاصه + جدول همه کسب‌وکارها |
| `GET /admin/business/:id` | جزئیات کامل read-only یک کسب‌وکار |

### داشبورد کل (`/admin`)

**کارت‌های خلاصه کل سیستم:**
- تعداد کسب‌وکارهای فعال
- تعداد لایسنس‌های کل / استفاده‌شده / آزاد
- تعداد کل کاربران سیستم
- فروش کل همه کسب‌وکارها در این ماه
- مخارج کل این ماه
- بدهی کل به تأمین‌کنندگان (all-time)
- مانده حقوق پرسنل (all-time)

**جدول کسب‌وکارها:**
نام • نوع • شهر • مالک (نام + telegram_id) • وضعیت • تاریخ ثبت • تعداد اعضا •
فروش ماه • مخارج ماه • مانده نقدی • بدهی • مانده پرسنل • لینک جزئیات

### صفحه جزئیات (`/admin/business/:id`)

| بخش | محتوا |
|-----|-------|
| مشخصات کسب‌وکار | نام، نوع، شهر، تلفن، وضعیت، تاریخ ثبت |
| مشخصات مالک | نام، telegram_id |
| وضعیت لایسنس | کد، فعال/غیرفعال، تاریخ فعال‌سازی |
| کارت‌های مالی | فروش، مخارج، بدهی، انبار، مانده نقدی، پس از تعهدات |
| اعضای تیم | نام، نقش، تلگرام، تاریخ عضویت |
| شعبه‌ها | نام، آدرس، وضعیت |
| فروش‌های اخیر | آخرین ۲۰ فروش |
| مخارج اخیر | آخرین ۲۰ مخرج |
| تأمین‌کنندگان | همه + بدهی‌ها |
| خریدهای مواد | آخرین ۲۰ خرید |
| پرداخت‌های تأمین‌کننده | آخرین ۲۰ پرداخت |
| موجودی انبار | همه اقلام + هشدار کم‌موجودی |
| حساب پرسنل | همه پرسنل + مانده |

### امنیت و حریم خصوصی

- `business_owner` داده‌های خودش را در `/` و سایر مسیرهای عادی می‌بیند — **هرگز** داده سایر کسب‌وکارها را نمی‌بیند
- `/admin` و `/admin/business/:id` فقط با `requireSuperAdmin` middleware محافظت می‌شوند
- همه عملیات read-only هستند — هیچ INSERT/UPDATE/DELETE از داشبورد انجام نمی‌شود

### Service Layer

`src/core/adminService.js` — همه queryهای ادمین:
```
getSystemOverview()               — خلاصه کل سیستم
listBusinessesWithFinancialSummary() — لیست + آمار مالی
getBusinessAdminDetail(id)        — مشخصات + تیم + لایسنس
getBusinessRecentSales(id)        — آخرین ۲۰ فروش
getBusinessRecentExpenses(id)     — آخرین ۲۰ مخرج
getBusinessRecentPurchases(id)    — آخرین ۲۰ خرید مواد
getBusinessRecentSupplierPayments(id) — آخرین ۲۰ پرداخت
getBusinessInventorySummary(id)   — موجودی انبار
getBusinessPayrollSummary(id)     — خلاصه حقوق پرسنل
```
...
| Phase 9B | Super Admin Dashboard — پنل کل سیستم، جزئیات هر کسب‌وکار | ✅ |
| Phase 9C+ | خروجی CSV از داشبورد، Bale | 🔜 |
